...
"""
import requests  # pip install types-requests requests
...
flags.add_argument(
    ...
    help='server url to get next one proxy server url',
)
...
class ProxyPoolPlugin(TcpUpstreamConnectionHandler, HttpProxyBasePlugin):
    ...
    def _select_proxy(self) -> Url:
        ...
        """
        return Url.from_bytes(requests.get(self.flags.proxy_url_server).content)
